...
df = pd.read_csv('metabolomics_withoutPA/Data/initial_tstats.csv', index_col=0)

#Get number of array job, which corresponds to one of the folders that contain 10k permutation values
index_num = sys.argv[1]
path = os.getcwd() + '/metabolomics_withoutPA/Results' + str(index_num) 
...
with open ('metabolomics_withoutPA/Data/vals'+str(index_num)+'.txt', 'w') as file:
